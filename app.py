...
# ================= AUDIO PREPROCESS =================
def preprocess_audio(file_path, duration=8):
...
def predict_audio():
    ...
    try:
        ...
        if isinstance(processed, str):
            print("Processed is string:", processed)
        else:
            print("SHAPE:", processed.shape)

        if isinstance(processed, str) and processed == "long":
            os.remove(path)
            return jsonify({
                "error": "Audio must be less than 20 seconds."
            })

        prediction = model_audio.predict(processed)[0]
